...
print(
    "TOKEN_SGA:",
    "OK" if token_sga else "AUSENTE"
)

print(
    "USUARIO_API:",
    "OK" if usuario else "AUSENTE"
)

print(
    "SENHA_API:",
    "OK" if senha else "AUSENTE"
)

if not token_sga:
    raise SystemExit(
        "❌ TOKEN_SGA não foi encontrado nas variáveis de ambiente."
    )

if not usuario:
    raise SystemExit(
        "❌ USUARIO_API não foi encontrado nas variáveis de ambiente."
    )

if not senha:
    raise SystemExit(
        "❌ SENHA_API não foi encontrada nas variáveis de ambiente."
    )
...
# ============================================================
# 4 FAZER REQUISIÇÃO POST PARA GERAR TOKEN_USUARIO
# ============================================================

print("🔐 Tentando autenticar na API Hinova...")
...
# ============================================================
# 5 TRATAR RETORNO DA AUTENTICAÇÃO
# ============================================================
...
if resposta.status_code == 200:
    ...
    try:
        ...
        print("❌ API retornou 200, mas a resposta não é um JSON válido.")
        print("Resposta recebida:", resposta.text)

        raise SystemExit(1)

    token_usuario = (
        dados.get("token_usuario")
        or dados.get("token_usuário")
    )
    ...
    if token_usuario:
        ...
        print(
            "❌ Autenticação retornou 200, "
            "mas nenhum token foi encontrado."
        )
    ...
    print(
        f"❌ Erro {resposta.status_code} na autenticação."
    )

    print("📩 Resposta da API:")

    # Mostra a resposta para descobrirmos o motivo do 403
    print(resposta.text)
...
def coletar_periodo_total(data_inicio_str):
    ...
    while current_start <= data_fim:
        ...
        current_end = min(
            current_start + timedelta(days=delta_dias - 1),
            data_fim
        )
...
if not df.empty:
    ...
    df = df.apply(
        lambda x: x.str.encode(
            'utf-8',
            'ignore'
        ).str.decode('utf-8')
    )

    enviar_para_sheets_em_lotes(
        service,
        SPREADSHEET_ID,
        SHEET_NAME,
        df
    ...
    print(
        "✅ Enviado ao Google Sheets com sucesso."
    )
...
if response.status_code != 200:
    ...
    raise Exception(
        f"Erro {response.status_code} ao buscar situações. "
...
print(
    f"✅ Total de situações coletadas: {len(df)}"
)
...
print(
    "✅ DIM_SITUACAO enviada ao Google Sheets."
)


# ============================================================
# VOLUNTARIOS - ATIVOS VINDOS DO SGA
# ============================================================
...
URL_SGA = (
    "https://api.hinova.com.br/"
    "api/sga/v2/listar/voluntario/ativo"
)
...
def get_voluntarios():
    ...
    while True:
        ...
        print(
            f"✅ Página {pagina} coletada, "
            f"registros nesta página: {qtd_retorno}, "
            f"total acumulado: {len(voluntarios)}"
        )
...
def transformar_voluntarios_em_dataframe(
    voluntarios
):
    ...
    for v in voluntarios:
        ...
        linha = {
            ...
            "formato_pagamento": v.get(
                "formato_pagamento"
            ),

            "valor_pagamento": v.get(
                "valor_pagamento"
            ),

            "formato_pagamento_residual": v.get(
                "formato_pagamento_residual"
            ),

            "valor_pagamento_residual": v.get(
                "valor_pagamento_residual"
            ),

            "codigo_classificacao": v.get(
...
df = transformar_voluntarios_em_dataframe(
    voluntarios_totais
)
...
SHEET_NAME = "SITUAÇÃO DE EVENTO ATIVOS SGA"
...
if resposta.status_code == 200:
    ...
    print(
        ...
        f"na requisição de situação de evento."
    )
...
if not df.empty:
    ...
    print(
        "⚠️ DataFrame vazio, nada enviado "
...
def transformar_veiculos_em_dataframe(
    veiculos
):
    ...
    for v in veiculos:
        ...
        linha = {
            ...
            "codigo_veiculo": v.get(
                "codigo_veiculo"
            ),

            "codigo_associado": v.get(
                "codigo_associado"
            ),
            ...
            "data_contrato_final": v.get(
                "data_contrato_final"
            ),

            # --- ORGANIZACIONAL ---

            "codigo_regional": v.get(
                "codigo_regional"
            ),

            "codigo_cooperativa": v.get(
                "codigo_cooperativa"
            ),
            ...
            "nome_voluntario": v.get(
                "nome_voluntario"
            ),

            # --- ASSOCIADO ---

            "nome_associado": v.get(
                "nome_associado"
            ),
...
for codigo in codigos_situacao:
    ...
    print(
        f"⏳ Coletando veículos "
        f"da situação {codigo}..."
    )

    veiculos = get_veiculos_por_situacao(
        codigo
    )

    print(
        f"✅ {len(veiculos)} veículos encontrados"
    )

    todos_veiculos.extend(
        veiculos
    )


df = transformar_veiculos_em_dataframe(
    todos_veiculos
)
...
SHEET_NAME = "Dados de Criação CRM"
...
Token_CRM = os.environ.get(
    "TOKEN_CRM"
)

URL_CRM = (
    "https://api.powercrm.com.br/api/report/db"
)
...
while current_start < end_date:
    ...
    current_end = min(
        current_start + timedelta(days=delta_days - 1),
        end_date
    )
...
if not df.empty:
    ...
    print(
        "⚠️ DataFrame vazio, "
        "nada enviado para o Google Sheets."
    )
